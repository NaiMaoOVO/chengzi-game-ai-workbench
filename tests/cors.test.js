...
test("parseAllowedOrigins keeps the shared default whitelist and trims entries", () => {
  assert.equal(parseAllowedOrigins().has("null"), true);
...
test("isOriginAllowed treats no Origin, literal null, whitelist and wildcard as allowed", () => {
  const cors = createCors({ allowedOrigins: "null,http://ok.example", methods: "GET, OPTIONS" });
  assert.equal(isOriginAllowed(request(), cors.allowedOrigins), true);
  assert.equal(isOriginAllowed(request("null"), cors.allowedOrigins), true);
...
test("corsHeaders echoes allowed origins and answers disallowed ones with fixed null ACAO", () => {
  ...
  assert.equal(anonymous["Access-Control-Allow-Origin"], "null");
...
test("all service presets share the same default origin whitelist", () => {
  ...
  for (const origin of ["null", "http://localhost:8793", "http://127.0.0.1:8793", "http://localhost:3000", "http://localhost:5173"]) {
