...
function createRetryBudget({ capacity = 3, refillIntervalMs = 1000 } = {}) {
  let tokens = capacity;
  let lastRefill = Date.now();
  return {
    trySpend() {
      const now = Date.now();
      tokens = Math.min(capacity, tokens + Math.floor((now - lastRefill) / refillIntervalMs));
      lastRefill = now;
...
module.exports = { createRateLimiter, stableSerialize, createSingleFlightCache, createRetryBudget };
